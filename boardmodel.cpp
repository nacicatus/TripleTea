...
//! [loader]
bool BoardModel::loadFile(const QString &fileName)
{
    QFile file(fileName);
...
void BoardModel::loadPattern(const QString &plainText)
{
    clear();
...
int BoardModel::cellNeighborsCount(const QPoint &cellCoordinates) const
{
    int count = 0;
...
bool BoardModel::areCellCoordinatesValid(const QPoint &coordinates)
{
    return QRect(0, 0, width, height).contains(coordinates);
}

QPoint BoardModel::cellCoordinatesFromIndex(int cellIndex)
{
    return {cellIndex % width, cellIndex / width};
}

std::size_t BoardModel::cellIndex(const QPoint &coordinates)
{
